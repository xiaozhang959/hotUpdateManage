...
export default function ProfileStoragePage() {
  ...
  const Hint = ({ text }: { text: string }) => (
    <span title={text} className="inline-flex items-center ml-1 align-middle cursor-help">
    ...
    switch (p) {
      case 'LOCAL': return (
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label>publicPrefix<Hint text="URL 前缀。必须以 / 开头且不以 / 结尾。默认 /uploads。注意：next.config.ts 只配置了 /uploads → /api/uploads 的重写，如修改需同步修改 rewrites。"/></Label>
          ...
          <div>
            <Label>baseDir 子目录<Hint text="可选，位于 uploads 下的子目录名称（相对路径，不要以 / 开头或结尾）。例如填写 ddd，则文件将存储在 uploads/ddd/...。留空表示直接存储在 uploads/...。"/></Label>
      ...
      case 'WEBDAV': return (
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label>baseUrl<Hint text="WebDAV 根地址，末尾不要带 /。示例：Nextcloud → https://cloud.example.com/remote.php/webdav；通用 → https://dav.example.com/webdav。"/></Label>
          ...
          <div>
            <Label>publicBaseUrl<Hint text="可选，对外匿名可访问的域名（如 CDN）。留空则与 baseUrl 相同；如外网不可匿名访问，建议留空，下载将通过服务端代理。"/></Label>
          ...
          <div>
            <Label>username<Hint text="如 WebDAV 服务启用认证，请填写用户名。"/></Label>
          ...
          <div>
            <Label>password<Hint text="如 WebDAV 服务启用认证，请填写密码。"/></Label>
          ...
          <div>
            <Label>rootPath<Hint text="仓库子目录（可选）。不要以 / 开头或结尾，例如 hotupdates。最终路径：baseUrl/rootPath/{projectId}/{file}。"/></Label>
      ...
      case 'S3': return (
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label>region<Hint text="AWS：真实区域（如 ap-northeast-1）。S3 兼容（MinIO/Bitiful/R2）：常用 us-east-1 或服务要求值。"/></Label>
          ...
          <div>
            <Label>endpoint<Hint text="S3 兼容服务必填（如 MinIO/Bitiful），如 https://s3.bitiful.net；AWS 官方可留空。"/></Label>
          ...
          <div>
            <Label>publicBaseUrl<Hint text="可选，公开/CDN 域名。桶私有时留空，下载将通过服务端预签名。"/></Label>
          ...
          <div className="col-span-2 flex items-center gap-2">
            ...
            <Label htmlFor="forcePathStyle">forcePathStyle（路径式地址）<Hint text="MinIO/部分兼容服务需要勾选。勾选后 URL 形如 https://endpoint/bucket/key；不勾选形如 https://bucket.endpoint/key。"/></Label>
      ...
      case 'OSS': return (
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <Label>accessKeySecret<Hint text="访问密钥 Secret。注意保护，不要泄露。"/></Label>
          ...
          <div>
            <Label>endpoint<Hint text="可选，自定义域名或内网地址。留空使用官方域名。"/></Label>
            <Input value={form.config?.endpoint||''} onChange={e=>setForm({...form, config:{...form.config, endpoint:e.target.value}})} placeholder="可选"/>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-3xl">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>名称<Hint text="自定义显示名称，用于页面展示和区分不同存储配置。"/></Label>
                <Input value={form.name} onChange={e=>setForm({...form, name:e.target.value})} />
              </div>
              <div>
                <Label>Provider<Hint text="存储类型：LOCAL/WEBDAV/S3/OSS。选择后按下方字段填写对应配置。"/></Label>
