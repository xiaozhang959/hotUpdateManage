...
async function createTransporter() {
  ...
  return (nodemailer as any).createTransport(transporterConfig)
}
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          ${actionUrl ? `
            <div style="text-align: center;">
              <a href="${actionUrl}" class="button">${actionText || '查看详情'}</a>
            </div>
          ` : ''}
        </div>
        <div class="footer">
...
export async function testSMTPConnection(): Promise<{ success: boolean; message: string }> {
  try {
    ...
    const transporter = (nodemailer as any).createTransport(transporterConfig)
